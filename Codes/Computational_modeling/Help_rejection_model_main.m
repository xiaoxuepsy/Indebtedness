...
P = importdata(fullfile(basedir, 'Kappa.txt')); 
...
% The first colume is subejct ID, and the second colume is the value of kappa
para = P.data;
...
%--------------------------------------------------------------------------
% Create class instance for example lin_linear1ear model
%--------------------------------------------------------------------------
%
...
% example 'lin_linear1ear_model' function below.
%
...
lin_linear = comp_model(data,dat.textdata,'Help_rejection_model_function','nStart',1000, 'param_min',[0, -1, 0], 'param_max', [1,1,50], 'esttype','LLE');

%   911x8 comp_model array with properties:
% 
%         model: 'lin_linear1ear_model'
%     param_min: [-5 -20]
%     param_max: [60 20]
%        nStart: 10
%       esttype: 'SSE'
...
lin_linear = lin_linear.fit_model();
...
% Summary of Model: lin_linear1ear_model
% -----------------------------------------
...
lin_linear.write_tables(fullfile(basedir,'Results'))
...
lin_linear.save(fullfile(basedir,'Results'))
...
% plot(lin_linear1, [3,4], 'title', 'Linear Model', 'xlabel','cost', 'ylabel', 'Average reciprocity', 'legend', {'Predicted','Observed'})
